<div class="grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-6">
    <div class="bg-white rounded-xl shadow p-5 sm:p-6 border-l-4 border-blue-900">
        <p class="text-sm text-gray-500">Aplicações</p>
        <p class="text-3xl font-bold text-gray-800 mt-2"><?= h($appCount) ?></p>
    </div>
    <div class="bg-white rounded-xl shadow p-5 sm:p-6 border-l-4 border-orange-500">
        <p class="text-sm text-gray-500">Contatos N2</p>
        <p class="text-3xl font-bold text-gray-800 mt-2"><?= h($contactCount) ?></p>
    </div>
    <div class="bg-white rounded-xl shadow p-5 sm:p-6 border-l-4 border-green-500">
        <p class="text-sm text-gray-500">Sobreavisos</p>
        <p class="text-3xl font-bold text-gray-800 mt-2"><?= h($scheduleCount) ?></p>
    </div>
</div>

<div class="mt-6 sm:mt-8 bg-white rounded-xl shadow p-5 sm:p-6">
    <h2 class="text-xl font-bold text-gray-800 mb-4">Atalhos</h2>
    <div class="flex flex-wrap gap-3">
        <a href="/applications/new" class="bg-blue-900 text-white px-4 py-2 rounded w-full sm:w-auto text-center">Nova Aplicação</a>
        <a href="/contacts/new" class="bg-orange-500 text-white px-4 py-2 rounded w-full sm:w-auto text-center">Novo Contato</a>
        <a href="/schedule/new" class="bg-green-600 text-white px-4 py-2 rounded w-full sm:w-auto text-center">Nova Escala</a>
    </div>
</div>

    <div class="mt-6 sm:mt-8 bg-white rounded-xl shadow p-5 sm:p-6 overflow-hidden">
            ...
            <table class="table-stack min-w-full leading-normal">
                ...
                <tbody>
                    <?php foreach ($todayOnCalls as $oc): ?>
                        <tr>
                            <td data-label="Analista" class="px-5 py-4 border-b border-gray-200 bg-white text-sm font-bold text-gray-800"><?= h($oc['analyst_name']) ?></td>
                            <td data-label="Telefone" class="px-5 py-4 border-b border-gray-200 bg-white text-sm"><?= h($oc['phone']) ?></td>
                            <td data-label="Início" class="px-5 py-4 border-b border-gray-200 bg-white text-sm"><?= h(date('d/m/Y H:i', strtotime($oc['start_date']))) ?></td>
                            <td data-label="Fim" class="px-5 py-4 border-b border-gray-200 bg-white text-sm"><?= h(date('d/m/Y H:i', strtotime($oc['end_date']))) ?></td>
                            <td data-label="Observação" class="table-full px-5 py-4 border-b border-gray-200 bg-white text-sm"><?= h($oc['observation']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
