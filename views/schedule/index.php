<div class="bg-white shadow-md rounded my-6 overflow-hidden">
    <div class="p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-gray-50 border-b">
        ...
        <a href="/schedule/new" class="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded text-sm inline-flex items-center w-full sm:w-auto justify-center"><i class="fas fa-calendar-plus mr-2"></i>Adicionar Escala</a>
    </div>
            ...
            <tbody>
                <?php if ($events): ?>
                    <?php foreach ($events as $event): ?>
                        <tr>
                            <td data-label="Analista" class="px-5 py-5 border-b border-gray-200 bg-white text-sm font-bold"><?= h($event['analyst_name']) ?></td>
                            <td data-label="Telefone" class="px-5 py-5 border-b border-gray-200 bg-white text-sm"><?= h($event['phone']) ?></td>
                            <td data-label="Início" class="px-5 py-5 border-b border-gray-200 bg-white text-sm"><?= h(date('d/m/Y H:i', strtotime($event['start_date']))) ?></td>
                            <td data-label="Fim" class="px-5 py-5 border-b border-gray-200 bg-white text-sm"><?= h(date('d/m/Y H:i', strtotime($event['end_date']))) ?></td>
                            <td data-label="Observação" class="table-full px-5 py-5 border-b border-gray-200 bg-white text-sm"><?= h($event['observation']) ?></td>
                            <td class="table-actions px-5 py-5 border-b border-gray-200 bg-white text-sm text-center whitespace-nowrap">
